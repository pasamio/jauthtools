...
require_once(JPATH_BASE.DS."administrator/components/com_sso/classes/ssoProvider.class.php");

// include language file
if (is_file(JPATH_BASE.DS."components/com_sso/language/$mosConfig_lang.php")) {
	include_once (JPATH_BASE.DS."components/com_sso/language/$mosConfig_lang.php");
} else {
	include_once (JPATH_BASE.DS."components/com_sso/language/english.php");
}
...
<br />
	...
	if (! $sps) {
		...
		?>
<form name="form_mod_sso"><select name="sp" class="inputbox">
	<option value=""><?php echo _SSO_PLEASE_SELECT ?></option>
	<?php
	foreach ($sps as $sp) {
		?>
	<option value="<?php echo urlencode($sp->providerId) ?>"
		title="<?php echo $sp->siteName ?>"><?php echo substr($sp->siteName, 0, 16) ?></option>
		<?php
	}
	?>
</select>
<button class="button"
	onclick="javascript:if (form_mod_sso.sp.value) document.location.href='<?php echo $idp->providerId ?>/index.php?option=com_sso&task=idplogin&sp=' + form_mod_sso.sp.value; return false;"><?php echo _SSO_MAKE_SSO ?></button>
</form>
	<?php
	}
	else {
		...
		<ul>
			...
			foreach ($sps as $sp) {
				?>
				<li><a
					href="<?php echo $idp->providerId ?>/index.php?option=com_sso&task=idplogin&sp=<?php echo urlencode($sp->providerId) ?>"
					title="<?php echo $sp->siteName ?>"><?php echo substr($sp->siteName, 0, 16) ?></a></li>
					<?php
			}
			?>
		</ul>
	...
	if (! $idps) {
		...
		<form name="form_mod_sso"><select name="idp" class="inputbox">
			<option value=""><?php echo _SSO_PLEASE_SELECT ?></option>
			<?php
			foreach ($idps as $idp) {
				?>
				<option value="<?php echo $idp->providerId ?>"
					title="<?php echo $idp->siteName ?>"><?php echo substr($idp->siteName, 0, 16) ?></option>
				<?php
			}
			?>
		</select>
		<button class="button"
			onclick="javascript:if (form_mod_sso.idp.value) document.location.href=form_mod_sso.idp.value + '/index.php?option=com_sso&task=idplogin&sp=<?php echo urlencode($mosConfig_live_site) ?>'; return false;"><?php echo _SSO_LOGIN ?></button>
		form>
			<?php
	}
	else {
		...
		foreach ($idps as $idp) {
			?>
			<li><a
				href="<?php echo $idp->providerId ?>/index.php?option=com_sso&task=idplogin&sp=<?php echo urlencode($mosConfig_live_site) ?>"
				title="<?php echo $idp->siteName ?>"><?php echo substr($idp->siteName, 0, 16) ?></a></li>
				<?php
		}
		?>
		</ul>
